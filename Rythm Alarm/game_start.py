...
class game_start():
    ...
    def __init__(self, root):
        ...
        self.fumen_list = self.read_fumen(
            './assets/'+self.settings[2]+'.score')
        ...
        self.frame_jubeat = JubeatFrame.JubeatFrame(
            master=root, score=self.score, parent=self, time=self.timer)
        self.frame_jubeat.grid(
            row=0, column=0, sticky="nsew", pady=20, padx=20)
    ...
    def repeat_game(self):
        ...
        for i in range(1, int(self.fumen_list[0][0])+1):
            ...
            self.after_id_list.append(self.frame_jubeat.after(
                str(int(self.fumen_list[i][0])-self.delay), self.panel_bright, i))
        self.after_id_list.append(self.frame_jubeat.after(
            self.fumen_list[0][1], self.repeat_game))
    # ゲームを停止する関数
    ...
    def stop_game(self):
        ...
        # クリアタイムを計算
        clear_m = str(math.floor(
            (86400-Alarm.calc_time_delta(self.settings[0], self.settings[1])) / 60))
        clear_s = str(
            (86400-Alarm.calc_time_delta(self.settings[0], self.settings[1])) % 60).zfill(2)
